...
Google discovers new pages primarily by following links from pages it already knows about. When a page has no internal links, Google has fewer ways to find it.

Even if the page is in your XML sitemap, Google prioritizes pages that are well-connected within your site structure. An orphaned page sends a signal that it's not important enough to link to—so Google may not prioritize indexing it.
...
Crawl budget is the number of pages Google will crawl on your site within a given timeframe. For most small sites, this isn't a concern. But for larger sites with thousands of pages, crawl efficiency matters.
...
When your homepage gets backlinks from other websites, some of that authority flows through internal links to your other pages. But orphan pages are cut off from this flow entirely.

Even if you've created a great piece of content, it can't rank to its potential without internal links supporting it. You're essentially handicapping your own content.
...
Use a 301 (permanent) redirect to pass link equity to the new destination. This works well for:
...
If you want these pages to stay live but not appear in search results, add a noindex tag:
